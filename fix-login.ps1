...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (user.status === 'suspended') return unauthorized('Tu cuenta esta suspendida.');
    ...
    const plan = (user.plan === 'free' || !user.plan_expires_at) ? user.plan :
      (new Date(user.plan_expires_at) < new Date() ? 'free' : user.plan);

    await env.DB.prepare('UPDATE users SET last_login_at = datetime(\'now\'), plan = ? WHERE id = ?')
    ...
    return ok(
      { message: 'Login correcto', plan: plan, name: user.name },
...
'@
Set-Content -Path "functions\api\auth\login.js" -Encoding UTF8 -Value $content
Write-Host "Archivo escrito OK"